...
- **Purpose**: This node acts as an entry point to fetch website data. It's designed to respond to incoming webhook requests to retrieve information from the "🧩 Websites" table in Airtable.

### 2. Websites
- **Type**: Airtable - Get Data
- **Base**: Strategy (`appjHSW7sGtitxoHf`)
- **Table**: 🧩 Websites (`tblkCFFIUhSA36DK7`)
- **Operation**: Search
...
- **Method**: Triggered via POST request.
- **Purpose**: This node serves as an endpoint to retrieve support options. It's configured to fetch data from the "🔑 Website Support Options" table in Airtable.

### 4. Support Options
- **Type**: Airtable - Get Data
- **Base**: Strategy (`appjHSW7sGtitxoHf`)
- **Table**: 🔑 Website Support Options (`tbliWJ8vTvhM0TUVv`)
- **Operation**: Search
...
- **Table**: 🧩 Websites (`tblkCFFIUhSA36DK7`)
- **Operation**: Update
- **Purpose**: Updates a specific website record in the "🧩 Websites" Airtable table. It uses the `websiteRecordId` from the incoming webhook data to identify the record and updates the "Website Support Options" field with the provided `supportOptionIds`.
...
3.  **Updating Website Support Options**: A `wsa-update-website-support-options` webhook, when triggered with a website ID and support option IDs, leads to an Airtable update operation on the "🧩 Websites" table.
